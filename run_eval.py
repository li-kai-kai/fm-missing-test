...
from fmexp.config import Config, SCENARIO_ORDER, PRIMARY_SCENARIO
...
from fmexp.metrics import case_metrics, gt_hd95_cache, paired_bootstrap, summarize
...
def evaluate_split(method, seed, which_ckpt, cfg, splits, split_name, cases,
    ...
    for cid in cases:
        ...
        noise = init_noise_for_case(cid, gt.shape, cfg.eval_seed, device) if method == "B" else None
        # 真实 mask 一侧的距离变换只算一次，供该病例三个场景共用
        gt_cache = gt_hd95_cache(gt)
...
def build_summary(rows):
    ...
    out = []
    keys = sorted({(r["model"], r["seed"], r["scenario"]) for r in rows})
    for model, seed, scen in keys:
        sub = [r for r in rows if r["model"] == model and r["seed"] == seed and r["scenario"] == scen]
        s = {
...
def rebuild_combined(out):
    ...
    """
    import glob
    cols = ["patient_id", "model", "seed", "scenario", "split", "ckpt"] + METRIC_COLS + \
           ["seconds", "peak_gpu_gb"]
    rows = []
    for p in sorted(glob.glob(os.path.join(out, "metrics_per_case_seed*.csv"))):
        rows.extend(coerce_row(r) for r in csv.DictReader(open(p)))
    if not rows:
        return [], [], {}
...
def main():
    ...
    for s in summary:
        print(f"{s['model']:>5} {s['seed']:>4} {s['scenario']:>8} "
